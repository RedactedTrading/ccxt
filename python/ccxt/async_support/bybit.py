...
class bybit(Exchange, ImplicitAPI):
    ...
    async def get_account_info(self):
        return await self.privateGetV5AccountInfo()
    ...
    # symbol: str, type: OrderType, side: OrderSide, amount, price=None, params={}
    async def create_orders(self, orders, category):
        ...
        for o in orders:
            ...
            if market['spot']:
                order_details['category'] = 'spot'
            elif market['linear']:
                order_details['category'] = 'linear'
            elif market['inverse']:
                order_details['category'] = 'inverse'
            elif market['option']:
                order_details['category'] = 'option'
            if market['spot'] and (type == 'market') and (side == 'buy'):
                ...
                if self.options['createMarketBuyOrderRequiresPrice']:
                    ...
                    if price is None and cost is None:
                        ...
                        order_details['qty'] = self.cost_to_precision(symbol, amount)
                else:
                    order_details['qty'] = self.cost_to_precision(symbol, amount)
            else:
                order_details['qty'] = self.amount_to_precision(symbol, amount)
            isMarket = lowerCaseType == 'market'
            isLimit = lowerCaseType == 'limit'
            if isLimit:
                order_details['price'] = self.price_to_precision(symbol, price)
            ...
            if postOnly:
                order_details['timeInForce'] = 'PostOnly'
            elif timeInForce == 'gtc':
                order_details['timeInForce'] = 'GTC'
            elif timeInForce == 'fok':
                order_details['timeInForce'] = 'FOK'
            elif timeInForce == 'ioc':
                order_details['timeInForce'] = 'IOC'
            ...
            if triggerPrice is not None:
                order_details['triggerPrice'] = self.price_to_precision(symbol, triggerPrice)
            elif isStopLossTriggerOrder or isTakeProfitTriggerOrder:
                order_details['triggerDirection'] = 2 if ascending else 1
                triggerPrice = stopLossTriggerPrice if isStopLossTriggerOrder else takeProfitTriggerPrice
                order_details['triggerPrice'] = self.price_to_precision(symbol, triggerPrice)
                order_details['reduceOnly'] = True
            elif isStopLoss or isTakeProfit:
                if isStopLoss:
                    stopLossTriggerPrice = self.safe_value_2(stopLoss, 'triggerPrice', 'stopPrice', stopLoss)
                    order_details['stopLoss'] = self.price_to_precision(symbol, stopLossTriggerPrice)
                if isTakeProfit:
                    takeProfitTriggerPrice = self.safe_value_2(takeProfit, 'triggerPrice', 'stopPrice', takeProfit)
                    order_details['takeProfit'] = self.price_to_precision(symbol, takeProfitTriggerPrice)
            if market['spot']:
                # only works for spot market
                if triggerPrice is not None:
                    order_details['orderFilter'] = 'StopOrder'
                elif stopLossTriggerPrice is not None or takeProfitTriggerPrice is not None or isStopLoss or isTakeProfit:
                    order_details['orderFilter'] = 'tpslOrder'
            clientOrderId = self.safe_string(params, 'clientOrderId')
            if clientOrderId is not None:
                order_details['orderLinkId'] = clientOrderId
            elif market['option']:
                # mandatory field for options
                order_details['orderLinkId'] = self.uuid16()
            # The above code is printing the current timestamp using the `pd.to_datetime()` function.
            ...
            order_details = self.extend(order_details, params)
            
            formatted_orders.append(order_details)

        ccxt_sending_ts = pd.to_datetime('now', utc=True)
        # print('sending ts: ', ccxt_sending_ts)
        response = await self.privatePostV5OrderCreateBatch({
            'request': formatted_orders,
            'category': category
        })
        # print(response)
        #
        ...
        return [
            ...
            # 'ccxt_exchange_ts': int(response['time'])
    ...
    async def universal_transfer(self, code: str, amount, from_id, to_id, fromAccount, toAccount, params={}):
        """
        transfer currency internally between wallets on the same account
        see https://bybit-exchange.github.io/docs/v5/asset/create-inter-transfer
        :param str code: unified currency code
        ...
        transferId = self.safe_string(params, 'transferId', self.uuid())
        accountTypes = self.safe_value(self.options, 'accountsByType', {})
        from_account_type = self.safe_string(accountTypes, fromAccount, fromAccount)
        to_account_type = self.safe_string(accountTypes, toAccount, toAccount)
        
        currency = self.currency(code)
        amountToPrecision = self.currency_to_precision(code, amount)
        request = {
            'transferId': transferId,
            'fromMemberId': from_id,
            'toMemberId': to_id,
        ...
        response = await self.privatePostV5AssetTransferUniversalTransfer(self.extend(request, params))
        #
        # {
        #     "retCode": 0,
        #     "retMsg": "success",
